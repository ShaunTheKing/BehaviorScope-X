...
BS_SCRIPTS = PROJECT_ROOT / "BehaviorScope_Y_minimal" / "shared_scripts" / "scripts"
sys.path.insert(0, str(BS_SCRIPTS))

from cropping_y import (  # noqa: E402
    NCropDetection,
    compute_fixed_square_xyxy,
    crop_coverage_conf,
    crop_resize_pad_rgb,
)
from data_y import load_n_manifest, validate_n_npz_payload  # noqa: E402
...
from utils.pose_features_y import extract_relational_features  # noqa: E402
...
def _default_pose_config() -> Path:
    return PROJECT_ROOT / "dlc-models-pytorch" / "iteration-0" / "MARS_DLC_SuperAnimal2026-06-06-trainset95shuffle2" / "train" / "pytorch_config.yaml"


def _default_snapshot(name: str) -> Path:
    return PROJECT_ROOT / "dlc-models-pytorch" / "iteration-0" / "MARS_DLC_SuperAnimal2026-06-06-trainset95shuffle2" / "train" / name
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--output_root", type=Path, default=PROJECT_ROOT / "BehaviorScope_Y_minimal" / "outputs" / "npz_cache" / "mars_full_video_dlc_topdown")
    p.add_argument("--manifest_path", type=Path, default=None)
    p.add_argument("--pose_config", type=Path, default=_default_pose_config())
    p.add_argument("--pose_snapshot", type=Path, default=_default_snapshot("snapshot-best-027.pt"))
    p.add_argument("--detector_snapshot", type=Path, default=_default_snapshot("snapshot-detector-best-004.pt"))
