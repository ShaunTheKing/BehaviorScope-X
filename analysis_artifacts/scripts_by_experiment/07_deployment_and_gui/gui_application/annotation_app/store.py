...
class AnnotationStore:
    ...
    def _apply_schema_migrations(self) -> None:
        ...
        if "created_at" not in project_columns:
            ...
            self.conn.execute(
                "UPDATE projects SET created_at = ? WHERE created_at = ''",
                (now,),
            )
        if "updated_at" not in project_columns:
            ...
            self.conn.execute(
                "UPDATE projects SET updated_at = ? WHERE updated_at = ''",
                (now,),
            )
        if "last_opened_at" not in project_columns:
            ...
            self.conn.execute(
                "UPDATE projects SET last_opened_at = ? WHERE last_opened_at = ''",
                (now,),
            )

        add_column("videos", "filename", "TEXT NOT NULL DEFAULT ''")
        add_column("videos", "sha256", "TEXT NOT NULL DEFAULT ''")
        add_column("videos", "file_size", "INTEGER NOT NULL DEFAULT 0")
        add_column("videos", "total_frames", "INTEGER NOT NULL DEFAULT 0")
        add_column("videos", "duration_ms", "INTEGER NOT NULL DEFAULT 0")
        add_column("videos", "width", "INTEGER NOT NULL DEFAULT 0")
        add_column("videos", "height", "INTEGER NOT NULL DEFAULT 0")
        add_column("videos", "codec", "TEXT NOT NULL DEFAULT ''")
        add_column("videos", "imported_at", "TEXT NOT NULL DEFAULT ''")
        add_column("videos", "last_position_ms", "INTEGER NOT NULL DEFAULT 0")
        add_column("videos", "status", "TEXT NOT NULL DEFAULT 'unseen'")
        add_column("videos", "split", "TEXT NOT NULL DEFAULT 'train'")
        add_column("videos", "notes", "TEXT NOT NULL DEFAULT ''")
        self._migrate_video_rows(now)

        add_column("behaviors", "slug", "TEXT NOT NULL DEFAULT ''")
        add_column("behaviors", "color", "TEXT NOT NULL DEFAULT ''")
        add_column("behaviors", "definition", "TEXT NOT NULL DEFAULT ''")
        add_column("behaviors", "hotkey", "TEXT")
        add_column("behaviors", "sort_order", "INTEGER NOT NULL DEFAULT 0")
        add_column("behaviors", "is_active", "INTEGER NOT NULL DEFAULT 1")
        add_column("behaviors", "created_at", "TEXT NOT NULL DEFAULT ''")
        add_column("behaviors", "updated_at", "TEXT NOT NULL DEFAULT ''")
        self._migrate_behavior_rows(now)

        add_column("annotations", "project_id", "INTEGER NOT NULL DEFAULT 0")
        add_column("annotations", "video_id", "INTEGER NOT NULL DEFAULT 0")
        add_column("annotations", "behavior_id", "INTEGER NOT NULL DEFAULT 0")
        add_column("annotations", "start_frame", "INTEGER NOT NULL DEFAULT 0")
        add_column("annotations", "end_frame", "INTEGER NOT NULL DEFAULT 0")
        add_column("annotations", "start_ms", "INTEGER NOT NULL DEFAULT 0")
        add_column("annotations", "end_ms", "INTEGER NOT NULL DEFAULT 0")
        add_column("annotations", "status", "TEXT NOT NULL DEFAULT 'draft'")
        add_column("annotations", "confidence", "REAL NOT NULL DEFAULT 1.0")
        add_column("annotations", "notes", "TEXT NOT NULL DEFAULT ''")
        add_column("annotations", "is_ambiguous", "INTEGER NOT NULL DEFAULT 0")
        add_column("annotations", "is_locked", "INTEGER NOT NULL DEFAULT 0")
        add_column("annotations", "created_at", "TEXT NOT NULL DEFAULT ''")
        add_column("annotations", "updated_at", "TEXT NOT NULL DEFAULT ''")
        add_column("annotations", "extracted_clip_path", "TEXT")
        add_column("annotations", "source_version", "TEXT NOT NULL DEFAULT ''")
        self._migrate_annotation_rows(now)
        self._migrate_legacy_spans(now)

    def _migrate_video_rows(self, now: str) -> None:
        for row in self.conn.execute("SELECT id, path, filename, sha256, imported_at FROM videos").fetchall():
            video_id = int(row["id"])
            path = Path(str(row["path"]))
            filename = str(row["filename"] or "") or path.name or str(row["path"])
            ...
            if path.is_file():
                try:
                    ...
                    if not sha:
                        sha = partial_sha256(path)
                except OSError:
                    file_size = 0
            if not sha:
                sha = f"legacy:{video_id}:{path}"
            self.conn.execute(
                """
                UPDATE videos
                SET filename = CASE WHEN filename = '' THEN ? ELSE filename END,
                    sha256 = CASE WHEN sha256 = '' THEN ? ELSE sha256 END,
                    file_size = CASE WHEN file_size = 0 THEN ? ELSE file_size END,
                    imported_at = CASE WHEN imported_at = '' THEN ? ELSE imported_at END
                WHERE id = ?
                """,
                (filename, sha, file_size, now, video_id),
            )

    def _migrate_behavior_rows(self, now: str) -> None:
        rows = self.conn.execute(
            "SELECT id, project_id, name, slug, color, sort_order, created_at, updated_at FROM behaviors ORDER BY project_id, id"
        ).fetchall()
        seen: dict[int, set[str]] = {}
        for index, row in enumerate(rows):
            behavior_id = int(row["id"])
            project_id = int(row["project_id"])
            used = seen.setdefault(project_id, set())
            slug = str(row["slug"] or "").strip() or slugify(str(row["name"]))
            base = slug
            suffix = 2
            while slug in used:
            ...
            self.conn.execute(
                ...
                UPDATE behaviors
                SET slug = ?,
                    color = CASE WHEN color = '' THEN ? ELSE color END,
                ...
                WHERE id = ?
                """,
                (slug, default_color(index), index, now, now, behavior_id),
            )
        self.conn.execute(
            "CREATE UNIQUE INDEX IF NOT EXISTS idx_behaviors_project_slug ON behaviors(project_id, slug)"
        )

    def _migrate_annotation_rows(self, now: str) -> None:
        self.conn.execute(
            "UPDATE annotations SET created_at = ? WHERE created_at = ''",
            (now,),
        )
        self.conn.execute(
            "UPDATE annotations SET updated_at = ? WHERE updated_at = ''",
            (now,),
        )
        self.conn.execute(
            """
            UPDATE annotations
            SET source_version = start_frame || ':' || end_frame || ':' || behavior_id
            WHERE source_version = ''
            """
        )

    def _migrate_legacy_spans(self, now: str) -> None:
        table = self.conn.execute(
            "SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'spans'"
        ).fetchone()
        if table is None:
            return
        for span in self.conn.execute(
            """
            SELECT s.id, s.project_id, s.video_id, s.behavior_id, s.start_s, s.stop_s,
                   s.review_state, s.notes, v.fps
            FROM spans s
            JOIN videos v ON v.id = s.video_id
            """
        ).fetchall():
            source_version = f"legacy_span:{int(span['id'])}"
            exists = self.conn.execute(
                "SELECT id FROM annotations WHERE source_version = ?",
                (source_version,),
            ).fetchone()
            if exists is not None:
                continue
            fps = float(span["fps"] or 30.0)
            start_ms = int(round(float(span["start_s"]) * 1000.0))
            end_ms = int(round(float(span["stop_s"]) * 1000.0))
            start_frame = max(0, int(round(float(span["start_s"]) * fps)))
            end_frame = max(start_frame, int(round(float(span["stop_s"]) * fps)) - 1)
            status = str(span["review_state"] or "draft").strip().lower()
            ...
            self.conn.execute(
                ...
                (project_id, video_id, behavior_id, start_frame, end_frame, start_ms, end_ms,
                 status, confidence, notes, is_ambiguous, is_locked, created_at, updated_at,
                 extracted_clip_path, source_version)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1.0, ?, 0, 0, ?, ?, NULL, ?)
                """,
                (
                    int(span["project_id"]),
                    int(span["video_id"]),
                    int(span["behavior_id"]),
                    start_frame,
                    end_frame,
                    start_ms,
                    end_ms,
    ...
    def _seed_default_settings(self, project_id: int) -> None:
        defaults = {
            "dataset_root": str((self.db_path.parent / "dataset_root").resolve()),
            "normalized_fps": "30",
            "min_clip_frames": "30",
            "resume_video_id": "",
