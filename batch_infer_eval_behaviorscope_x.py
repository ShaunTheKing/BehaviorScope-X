...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--model_path", default=str(PACKAGE_ROOT / "runs" / "behaviorscope_x" / "best_model_macro_f1.pt"))
    p.add_argument("--model_config", default=str(PACKAGE_ROOT / "runs" / "behaviorscope_x" / "config.json"))
    ...
    p.add_argument("--output_root", default=str(PACKAGE_ROOT / "runs" / "behaviorscope_x_eval"))
...
def main() -> int:
    ...
    infer_script = PACKAGE_ROOT / "infer_x.py"
    if not infer_script.is_file():
        infer_script = REPO_ROOT / "BehaviorScope-X" / "infer_x.py"
