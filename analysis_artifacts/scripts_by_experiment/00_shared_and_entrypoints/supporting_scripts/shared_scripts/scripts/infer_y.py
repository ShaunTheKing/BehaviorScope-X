...
The two roles use the same .pt file but at different layers:
  - Detection:    full pipeline (layers 0-23) including Pose head
  - Classifier:   backbone trunk only (layers 0-9) through SPPF
...
The script:
  * loads the trained model from a checkpoint produced by `train_n.py`
  * runs YOLO + tracking via `cropping_n.iterate_yolo_n_crops`
...
Standalone — does not modify the existing single-animal
`infer_behavior_lstm.py`.

Usage examples:

    # Single video, write CSV next to model
    python BehaviorScope\\infer_n.py ^
        --model_path  runs\\R5\\best_model_macro_f1.pt ^
        --model_config runs\\R5\\config.json ^
        --yolo_weights runs\\mars_pose\\v1\\weights\\best.pt ^
        --source experiment.mp4 ^
        --output  experiment.behavior.csv

    # Batch over a folder of videos with annotated MP4s
    python BehaviorScope\\infer_n.py ^
        --model_path  runs\\R5\\best_model_macro_f1.pt ^
        --yolo_weights runs\\mars_pose\\v1\\weights\\best.pt ^
        --source C:\\videos\\test_2\\ ^
        --output_dir C:\\predictions\\test_2\\ ^
        --output_video_dir C:\\predictions\\test_2\\annotated\\
...
try:
    from cropping_y import iterate_yolo_n_crops, NCropDetection
    from cropping import load_yolo_model, normalize_device
...
def _write_smoothed_frames_csv(
    ...
    # Pre-fix: uncovered frames silently became class-0 — on the current
    # MARS schema that is "attack", inflating false-positive attack frames
    # in any region the sliding window did not reach.
    bg_idx_for_init = 0
...
def _normalize_rgb_window(rgb_uint8: np.ndarray) -> torch.Tensor:
    """[T, H, W, 3] uint8 -> [T, 3, H, W] float32 (Ultralytics YOLO style: x/255 only).

    Must match the training-time normalization in data_y._normalize_rgb so the
    YOLO trunk sees the same input distribution at train and inference.
    """
...
def detections_to_window_batch(
    ...
    n_animals: int,
    num_keypoints: int,
    visual_feature_cache: Optional[dict[int, dict[str, torch.Tensor]]] = None,
) -> dict:
    ...
    H, W = dets[0].group_rgb.shape[:2]

    group_rgb = np.stack([d.group_rgb for d in dets], axis=0)  # [T, H, W, 3]

    animal_rgb = np.zeros((T, n_animals, H, W, 3), dtype=np.uint8)
    ...
    for t, d in enumerate(dets):
        N_d = min(d.animal_rgbs.shape[0], n_animals)
        ...
        animal_rgb[t, :N_d] = d.animal_rgbs[:N_d]
        if K_d > 0:
    ...
    # YOLO-style normalization (x/255 only) — must match data_y at training time.
    group_t = _normalize_rgb_window(group_rgb).unsqueeze(0)
    ar = animal_rgb.transpose(1, 0, 4, 2, 3).astype(np.float32) / 255.0
    animal_t = torch.from_numpy(ar).unsqueeze(0)
    ...
    if visual_feature_cache is not None:
        frame_ids = [int(d.frame_idx) for d in dets]
        if all(fid in visual_feature_cache for fid in frame_ids):
            first = visual_feature_cache[frame_ids[0]]
            if "group_feat" in first:
                out["group_feat"] = torch.stack(
                    [visual_feature_cache[fid]["group_feat"] for fid in frame_ids],
                    dim=0,
                ).unsqueeze(0)
            if "animal_feat" in first:
                out["animal_feat"] = torch.stack(
                    [visual_feature_cache[fid]["animal_feat"] for fid in frame_ids],
                    dim=0,
                ).unsqueeze(0)
    return out


def _encode_detection_visual_features(
    ...
    """Encode one detection frame's visual crops once for overlapping windows."""
    ...
    with torch.inference_mode(), _autocast("cuda", dtype=_infer_dtype, enabled=amp_enabled):
        want_group = not getattr(model, "disable_group_rgb", False)
        want_animals = bool(getattr(model, "per_animal_rgb_alive", False))
        if want_group or want_animals:
            crops: list[np.ndarray] = []
            if want_group:
                crops.append(det.group_rgb)
            if want_animals:
                animal_rgb = np.zeros((n_animals, *det.animal_rgbs.shape[1:]), dtype=np.uint8)
                n_copy = min(int(det.animal_rgbs.shape[0]), int(n_animals))
                if n_copy > 0:
                    animal_rgb[:n_copy] = det.animal_rgbs[:n_copy]
                crops.extend([animal_rgb[i] for i in range(int(n_animals))])
            crop_arr = np.stack(crops, axis=0)
            crop_t = _normalize_rgb_window(crop_arr).unsqueeze(1).to(dev)  # [M, 1, 3, H, W]
            crop_feat = model._encode_frames(crop_t).squeeze(1).detach()  # [M, D]
            offset = 0
            if want_group:
                features["group_feat"] = crop_feat[offset]
                offset += 1
            if want_animals:
                features["animal_feat"] = crop_feat[offset: offset + int(n_animals)]
    return features
...
def _materialize_embedded_yolo_weights(
    checkpoint: dict,
    checkpoint_path: Path | str,
    cfg: dict,
) -> str | None:
    yolo_value = str(cfg.get("yolo_weights") or "")
    if not yolo_value.startswith("<embedded:"):
        return None
    bundle = checkpoint.get("embedded_yolo_weights") if isinstance(checkpoint, dict) else None
    ...
    filename = str(bundle.get("filename") or yolo_value.removeprefix("<embedded:").removesuffix(">"))
    filename = Path(filename).name or "embedded_yolo.pt"
    cache_dir = Path(checkpoint_path).resolve().parent / ".embedded_yolo_cache"
    cache_dir.mkdir(parents=True, exist_ok=True)
    cache_path = cache_dir / filename
    expected_sha = str(bundle.get("sha256") or "")

    should_write = True
    if cache_path.is_file() and expected_sha:
        existing_sha = hashlib.sha256(cache_path.read_bytes()).hexdigest()
        should_write = existing_sha != expected_sha
    if should_write:
        cache_path.write_bytes(raw)

    if expected_sha:
        actual_sha = hashlib.sha256(cache_path.read_bytes()).hexdigest()
        if actual_sha != expected_sha:
            raise ValueError(
                f"Embedded YOLO extraction checksum mismatch for {cache_path}: "
                f"expected {expected_sha}, got {actual_sha}"
            )
    print(f"[infer] extracted embedded YOLO weights to {cache_path}", flush=True)
...
def load_model_from_checkpoint(
    ...
    # the CLI --yolo_weights flag, which is required by infer_y.py for the
    # detection step anyway. This means inference reuses the same YOLO
    # checkpoint for both pose detection AND visual feature extraction
    ...
    if str(yolo_weights_path).startswith("<embedded:"):
        embedded_path = _materialize_embedded_yolo_weights(checkpoint, checkpoint_path, cfg)
        if embedded_path:
            yolo_weights_path = embedded_path
        elif yolo_weights_fallback:
            yolo_weights_path = yolo_weights_fallback
        else:
            raise ValueError(
                "The checkpoint config references embedded YOLO weights, but "
                "no embedded byte payload was found. Pass --yolo_weights to "
                "use a separate YOLO-pose checkpoint."
            )
    cfg["_resolved_yolo_weights_path"] = str(yolo_weights_path)
    yolo_backbone_end_layer = int(cfg.get("yolo_backbone_end_layer", 10))

    model = MultiAnimalBehaviorSequenceClassifier(
        num_classes=num_classes,
        yolo_weights_path=yolo_weights_path,
        yolo_backbone_end_layer=yolo_backbone_end_layer,
        ...
        attention_proj_dim=int(cfg.get("attention_proj_dim", 0)),
        positional_encoding=str(cfg.get("positional_encoding", "none")),
        pose_fusion_dim=int(cfg.get("pose_fusion_dim", 128)),
        pose_fusion_strategy=str(cfg.get("pose_fusion_strategy", "gated_attention")),
        disable_visual_streams=bool(flags.get("disable_visual_streams", False)),
        disable_group_rgb=bool(flags.get("disable_group_rgb", False)),
        disable_per_animal_rgb=bool(flags.get("disable_per_animal_rgb", False)),
        ...
        precomputed_visual_dim=int(cfg.get("precomputed_visual_dim", 0)),
    )
    state = checkpoint
...
def run_inference_on_source(
    *,
    ...
    visual_feature_cache: dict[int, dict[str, torch.Tensor]] = {}
    use_visual_feature_cache = bool(getattr(model, "_visual_alive", False))
    ...
    print(f"[infer] source={src_repr}  num_frames={num_frames}  stride={window_stride}"
          + ("  +annotated_video" if want_video else ""), flush=True)
    ...
    try:
        for det in _detection_stream(
            ...
            if use_visual_feature_cache:
                with metrics.stage("feature_encode"):
                    visual_feature_cache[int(det.frame_idx)] = _encode_detection_visual_features(
                        model,
                        det,
                        n_animals,
                        device,
            ...
            if want_video and video_writer is not None and det.orig_frame_bgr is not None:
                if not smooth_bouts:
            ...
            while len(buffer) >= num_frames:
                ...
                batch = detections_to_window_batch(
                    ...
                    n_animals,
                    num_keypoints,
                    visual_feature_cache=visual_feature_cache if use_visual_feature_cache else None,
                )
                _infer_dtype = amp_dtype_torch if amp_dtype_torch is not None else torch.float16
                ...
                if smooth_bouts and want_video:
                ...
                if use_visual_feature_cache:
                    min_keep = int(buffer[0].frame_idx) if buffer else end_frame + 1
                    for fid in list(visual_feature_cache.keys()):
                        if fid < min_keep:
                            del visual_feature_cache[fid]
        ...
        if smooth_bouts and want_video and video_writer is not None and pending_dets:
            try:
...
def parse_args():
    ...
    p.add_argument("--yolo_weights", default=None, help="YOLO-pose .pt; optional when --model_path is a bundled single-model checkpoint.")
    ...
    p.add_argument("--calibration_json", default=None,
                   help="Path to calibration.json produced by "
                        "mars_to_behaviorscope_npz_full.py. When set, the "
...
def _load_calibration_lookup(calibration_json: str) -> dict:
    ...
    mars_to_behaviorscope_npz_full.py and return a dict keyed by video_id.
...
def _lookup_body_length(calib: dict, source_stem: str):
    ...
    The MARS pipeline names YOLO outputs like
    'Mouse155_20151124_20-56-05_Top_J85.mp4' but the calibration keys are
    bare video IDs like 'Mouse155_20151124_20-56-05'. Match by prefix.
    """
...
def main():
    ...
    print(f"[infer] backbone={cfg.get('backbone','?')}  classes={list(idx_to_class.values())}", flush=True)
    ...
    yolo_weights_for_detection = args.yolo_weights or cfg.get("_resolved_yolo_weights_path") or cfg.get("yolo_weights")
    if not yolo_weights_for_detection or str(yolo_weights_for_detection).startswith("<embedded:"):
        raise SystemExit(
            "No usable YOLO-pose checkpoint was resolved. Pass --yolo_weights "
            "or use a bundled checkpoint created by package_single_model_y.py."
        )

    model_yolo = load_yolo_model(yolo_weights_for_detection, device=device, task=args.yolo_task)
    yolo_num_keypoints = inspect_yolo_keypoint_count(yolo_weights_for_detection)
