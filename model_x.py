...
Hard-mask vs soft-gate semantics (locked in proposal §6, §8):
...
class YOLOFrameEncoder(nn.Module):
    ...
    feature vector per crop. The pose head (layer 23) and the FPN neck
    (layers 10-22) are discarded — we only want appearance features here.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if self._trainable:
            ...
            # Drops activation memory (~8-10 GB at batch=32 × 32 frames ×
...
class MultiAnimalBehaviorSequenceClassifier(nn.Module):
    ...
    def __init__(
        ...
        #   2. Allocating ~150–200 MB of GPU memory for SlowFast weights
        ...
        precomputed_visual_dim = int(precomputed_visual_dim or 0)
        self.visual_backend = str(visual_backend or "yolo")
        if self._visual_alive and precomputed_visual_dim > 0:
            self.frame_encoder = None
            feature_dim = precomputed_visual_dim
        elif self._visual_alive:
            if not yolo_weights_path:
                raise ValueError(
                    "Visual streams are enabled, but no YOLO-pose weights were provided. "
                    "For MobileNetV3 or DeepLabCut cached workflows, pass "
                    "precomputed_visual_dim from the feature-cache metadata."
                )
            ...
            # error deep inside cropping_x at inference time.
            ...
            if yolo_nkpt is not None and yolo_nkpt != self.num_keypoints:
                raise ValueError(
                    ...
                    f"These must match — either retrain the YOLO model with "
        ...
        self.backbone_name = self.visual_backend
        ...
        if D_combined == 0:
            raise ValueError("All streams disabled — nothing to classify.")
    ...
    def _encode_frames(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.frame_encoder is None:
            raise RuntimeError(
                "This model was built for precomputed visual features. "
                "Provide group_feat/animal_feat from a matching feature cache "
                "instead of raw RGB crops."
            )
        if self.is_video_backbone:
    ...
    def forward(self, batch: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        if self.pose_fusion is not None:
            # Visual is alive — fuse visual+pose per animal.
            ...
            # Visual is dead (R4 / R4b) — the per-animal token *is* the
