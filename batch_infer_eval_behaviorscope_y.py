...
3. run BehaviorScope-Y/infer_y.py with calibrated crop scaling
...
NorPix .seq files is environment-dependent. MP4 conversion uses the project
SeqReader from scripts/mars_to_yolo_pose.py.
"""
...
from mars_to_yolo_pose import SeqReader  # noqa: E402
...
def discover_jobs(mars_root: Path, splits: Sequence[str]) -> list[VideoJob]:
    jobs: list[VideoJob] = []
    for split in splits:
        ...
        for video_dir in sorted([p for p in split_dir.iterdir() if p.is_dir()]):
            ...
            annot_path = find_annot(video_dir)
            if seq_path is None:
                print(f"[discover] skip {video_dir}: no .seq file", flush=True)
                continue
            if annot_path is None:
                print(f"[discover] skip {video_dir}: no human .annot file", flush=True)
                continue
            seek = seq_path.with_name(seq_path.stem + "-seek.mat")
            jobs.append(
                VideoJob(
                    split=split,
                    ...
                    seek_mat_path=seek if seek.is_file() else None,
                )
            )
    return jobs
        ...
        try:
            for idx in range(n_frames):
                ...
                if frame.ndim == 2:
                    ...
                    # PIL returns RGB; OpenCV writer expects BGR.
...
def evaluate_prediction_set(
    *,
    ...
    metrics25, matches25 = bout_metrics(gt_bouts, pred_bouts, 0.25)
    metrics50, matches50 = bout_metrics(gt_bouts, pred_bouts, 0.50)
    ...
    bout_macro25 = float(np.mean([metrics25[c]["f1"] for c in present_classes])) if present_classes else 0.0
    bout_macro50 = float(np.mean([metrics50[c]["f1"] for c in present_classes])) if present_classes else 0.0
    ...
    summary = {
        ...
        "bout_macro_f1_iou25_present_behaviors": bout_macro25,
        "bout_macro_f1_iou50_present_behaviors": bout_macro50,
        ...
        per_class_rows.append(
            {
                ...
                "frame_f1": f1[cls_idx],
                "bout_tp_iou25": metrics25[cls]["tp"],
                "bout_fp_iou25": metrics25[cls]["fp"],
                "bout_fn_iou25": metrics25[cls]["fn"],
                "bout_precision_iou25": metrics25[cls]["precision"],
                "bout_recall_iou25": metrics25[cls]["recall"],
                "bout_f1_iou25": metrics25[cls]["f1"],
                "bout_tp_iou50": metrics50[cls]["tp"],
                "bout_fp_iou50": metrics50[cls]["fp"],
                "bout_fn_iou50": metrics50[cls]["fn"],
                "bout_precision_iou50": metrics50[cls]["precision"],
                "bout_recall_iou50": metrics50[cls]["recall"],
                "bout_f1_iou50": metrics50[cls]["f1"],
    ...
    match_rows = []
    for row in matches25 + matches50:
        row = dict(row)
...
def resolve_cli_path(path_value: str | os.PathLike | None, *, base: Path = REPO_ROOT) -> Path | None:
    ...
    so normalize them here before creating outputs or spawning infer_y.py.
    """
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--mars_root", default=str(Path.cwd()))
    p.add_argument("--splits", nargs="+", default=["test_1", "test_2"])
    p.add_argument("--model_path", default=str(REPO_ROOT / "runs" / "behaviorscope_y" / "best_model_macro_f1.pt"))
    p.add_argument("--model_config", default=str(REPO_ROOT / "runs" / "behaviorscope_y" / "config.json"))
    ...
    p.add_argument("--output_root", default=str(REPO_ROOT / "runs" / "behaviorscope_y_eval"))
    p.add_argument("--source_mode", choices=["mp4", "seq"], default="mp4", help="'mp4' converts .seq before inference; 'seq' passes .seq directly to infer_y.py.")
...
def main() -> int:
    ...
    infer_script = REPO_ROOT / "BehaviorScope-Y" / "infer_y.py"
    ...
    jobs = discover_jobs(mars_root, args.splits)
    ...
    for job_i, job in enumerate(jobs, start=1):
        ...
        print(f"[batch] seq={job.seq_path}", flush=True)
        ...
        if args.source_mode == "mp4":
            try:
                ...
                if not args.dry_run and not resume_without_infer:
                    conversion_info = convert_seq_to_mp4(
                        job.seq_path,
                        converted_mp4,
        ...
        print(
            ...
            f"remaining≈{eta_min}m{eta_sec:02d}s",
            flush=True,
        )
