...
The script:
  * loads the trained model from a checkpoint produced by `train_x.py`
...
Usage examples:
    ...
    python BehaviorScope\\infer_x.py ^
        --model_path  runs\\behaviorscope_x\\best_model_macro_f1.pt ^
        --model_config runs\\behaviorscope_x\\config.json ^
        --yolo_weights runs\\pose\\weights\\best.pt ^
    ...
    python BehaviorScope\\infer_x.py ^
        ...
        --output_video_dir outputs\\predictions\\annotated\\
...
def _extract_embedded_yolo_weights(checkpoint: object, checkpoint_path: Path | str) -> str | None:
    if not isinstance(checkpoint, dict):
        return None
    payload = checkpoint.get("embedded_yolo_weights")
    if not isinstance(payload, dict):
        return None
    raw = payload.get("bytes")
    if not isinstance(raw, (bytes, bytearray)):
        return None
    digest = str(payload.get("sha256") or hashlib.sha256(raw).hexdigest())
    filename = str(payload.get("filename") or "embedded_yolo_pose.pt")
    safe_name = "".join(ch if ch.isalnum() or ch in ".-_" else "_" for ch in filename)
    cache_dir = Path(tempfile.gettempdir()) / "behaviorscope_x_embedded_yolo"
    cache_dir.mkdir(parents=True, exist_ok=True)
    out_path = cache_dir / f"{digest[:16]}_{safe_name}"
    if not out_path.exists() or out_path.stat().st_size != len(raw):
        out_path.write_bytes(bytes(raw))
    actual_digest = hashlib.sha256(out_path.read_bytes()).hexdigest()
    if actual_digest != digest:
        raise RuntimeError(
            f"Embedded YOLO checksum mismatch in {checkpoint_path}: "
...
try:
    from cropping_x import iterate_yolo_n_crops, NCropDetection
    ...
    from utils.pose_features_x import extract_per_animal_pose_features, REL_FEATURE_DIM
    from utils.inference_metrics import (
        MetricsLogger, NullMetricsLogger, derive_metrics_csv_path,
    )
    from config import IMAGENET_MEAN, IMAGENET_STD
    from annotate_x import annotate_frame_n
...
def _normalize_rgb_window(rgb_uint8: np.ndarray) -> torch.Tensor:
    ...
    Must match the training-time normalization in data_x._normalize_rgb so the
...
def detections_to_window_batch(
    ...
    # YOLO-style normalization (x/255 only) — must match data_x at training time.
...
def load_model_from_checkpoint(
    ...
    if config_json is not None:
        ...
        raise FileNotFoundError(
            "A train_x.py config.json or a checkpoint with embedded config is "
    ...
    feature_meta = cfg.get("feature_cache", {}).get("metadata", {})
    precomputed_visual_dim = int(
        cfg.get("precomputed_visual_dim")
        or feature_meta.get("feature_dim")
        or 0
    )
    visual_backend = str(
        feature_meta.get("visual_backend")
        or cfg.get("backbone")
        or "yolo"
    )
    ...
    if yolo_weights_path:
        cfg["yolo_weights"] = str(yolo_weights_path)
    ...
    model = MultiAnimalBehaviorSequenceClassifier(
        ...
        precomputed_visual_dim=precomputed_visual_dim,
    ...
    return model, cfg, idx_to_class, num_classes, n_animals, num_keypoints, checkpoint, str(yolo_weights_path or "")
    ...
    *,
    ...
    try:
        for det in _detection_stream(
            model_xolo, source, n_animals, crop_size,
...
def parse_args():
    ...
    p.add_argument("--calibration_json", default=None,
                   help="Path to calibration.json produced by "
                        ...
                        "video_id field. If omitted, infer_x.py first uses "
    ...
    p.add_argument("--decoder_config", default=None,
                   help="Optional decoder_config.json from train_x.py. If omitted, infer_x.py "
    ...
    p.add_argument("--temporal_splitter_config", dest="v25_splitter_config",
                   ...
                   help="Optional temporal_splitter.json sidecar. If omitted, infer_x.py "
...
def _load_calibration_lookup(calibration_json: str) -> dict:
    ...
    """
    try:
        with open(calibration_json, "r", encoding="utf-8-sig") as f:
            d = json.load(f)
    except Exception as exc:
...
def main():
    ...
    if device.startswith("cuda") and torch.cuda.is_available():
        try:
            ...
            pass
    model, cfg, idx_to_class, num_classes, n_animals, num_keypoints, checkpoint, yolo_weights_path = load_model_from_checkpoint(
    ...
    model_xolo = load_yolo_model(yolo_weights_path, device=device, task=args.yolo_task)
    yolo_num_keypoints = inspect_yolo_keypoint_count(yolo_weights_path)
    ...
    #   5. None -> per-frame median fallback inside cropping_x (with warning)
