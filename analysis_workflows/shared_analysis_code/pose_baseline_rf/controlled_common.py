...
def load_manifest_samples(manifest_path: Path, cfg: dict):
    ...
    from data_x import load_n_manifest  # noqa: WPS433
...
def feature_cache_path_for_sample(cache_dir: Path, sample, cfg: dict) -> Path:
    add_shared_scripts_to_path(cfg)
    from data_x import feature_cache_path  # noqa: WPS433
...
def extract_pose_relation_frame_features(npz_path: Path | str, cfg: dict) -> np.ndarray:
    """Return [T, D] explicit pose/relation features matching model_x inputs.
    ...
    """
    add_shared_scripts_to_path(cfg)
    from utils.pose_features_x import extract_per_animal_pose_features  # noqa: WPS433
