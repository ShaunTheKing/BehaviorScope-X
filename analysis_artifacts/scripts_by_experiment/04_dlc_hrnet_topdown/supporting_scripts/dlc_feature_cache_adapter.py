...
model and writes the cache contract consumed by ``train_y.py``:
...
def find_behaviorscope_scripts(manifest_path: Path) -> Path:
    for parent in [manifest_path.parent, *manifest_path.parents]:
        candidate = parent / "shared_scripts" / "scripts" / "data_y.py"
        if candidate.is_file():
            return candidate.parent
    for parent in manifest_path.parents:
        for candidate in parent.glob("*/shared_scripts/scripts/data_y.py"):
            return candidate.parent
    raise FileNotFoundError(
        "Could not locate BehaviorScope-Y shared_scripts/scripts/data_y.py "
...
def import_behaviorscope_helpers(manifest_path: Path):
    ...
    from data_y import feature_cache_path, load_n_manifest  # type: ignore
