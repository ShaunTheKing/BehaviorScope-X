...
def normalize_device(device: str | int | None) -> str:
    """
    Normalize GUI/CLI device values for both torch and Ultralytics.

    Ultralytics commonly accepts GPU ordinals such as "0", but
    torch.nn.Module.to() requires "cuda:0". Keep CPU strings unchanged.
    """
    if device is None:
        return "cpu"
