...
implementation milestones; legacy flags/config keys are still accepted for
...
def parse_args():
    ...
    p.add_argument("--yolo_weights", required=True,
                   help="Path to a YOLOv8/v11-pose .pt checkpoint trained on MARS. "
                        "The CSP-Darknet backbone (layers 0-9) is used as the visual "
                        "encoder.")
    p.add_argument("--yolo_backbone_end_layer", type=int, default=10,
                   help="Number of YOLO layers to keep as the visual backbone "
                        "(default 10 = layers 0-9, ending at SPPF).")
    p.add_argument("--train_backbone", action="store_true",
                   help="Fine-tune the YOLO backbone. Default: frozen "
                        "(use the YOLO weights as-is, since they are already "
                        "MARS-domain trained).")
    ...
    p.add_argument("--sequence_model", choices=["lstm", "attention", "tcn"], default="lstm")
    p.add_argument("--tcn_layers", type=int, default=5,
                   help="Number of dilated residual blocks for --sequence_model tcn.")
    p.add_argument("--tcn_kernel_size", type=int, default=3,
                   help="Temporal convolution kernel size for --sequence_model tcn.")
    p.add_argument("--use_attention_pool", action="store_true")
    p.add_argument("--attention_heads", type=int, default=4)
    p.add_argument("--attention_proj_dim", type=int, default=0,
                   help="Project combined features to this dim before attention (0=no projection)")
    ...
    p.add_argument("--disable_per_animal_rgb", action="store_true")
    ...
    p.add_argument("--use_feature_cache", "--user_feature_cache", dest="use_feature_cache", default=None,
                   help="Directory of precomputed YOLO visual features from "
                        "precompute_visual_features_y.py. When set, training loads "
                        "group_feat/animal_feat instead of re-running the frozen YOLO "
                        "visual trunk every epoch.")
    p.add_argument("--precomputed_visual_dim", type=int, default=0,
                   help="When >0, build the classifier for cached visual features "
                        "of this dimension and do not instantiate a YOLO visual "
                        "encoder. Used for non-YOLO frozen feature caches.")
    p.add_argument("--auto_feature_cache", action="store_true",
                   help="Automatically create or reuse a valid frozen-YOLO visual "
                        "feature cache before training. If --use_feature_cache is not "
                        "set, a yolo_feature_cache directory is created next to the "
                        "manifest.")
    p.add_argument("--allow_feature_cache_without_manifest", action="store_true",
                   help="Use an existing feature cache even if cache_manifest.json is "
                        "missing or stale. The trainer still verifies that every "
                        "required per-sample .npz cache file exists. Intended for "
                        "legacy caches that were built before cache metadata was "
                        "written.")
    p.add_argument("--feature_cache_batch", type=int, default=0,
                   help="Batch size for --auto_feature_cache precomputation. "
                        "0 reuses --batch.")
    p.add_argument("--feature_cache_num_workers", type=int, default=-1,
                   help="DataLoader workers for --auto_feature_cache precomputation. "
                        "-1 reuses --num_workers.")
    p.add_argument("--feature_cache_dtype", choices=["float32", "float16"], default="float32",
                   help="Saved dtype for --auto_feature_cache. float32 is safest; "
                        "float16 uses less disk.")
    # logging / diagnostics
    ...
    p.add_argument("--temporal_splitter_annot_root", dest="v25_splitter_annot_root",
                   ...
                   help="Optional directory of MARS .annot files for splitter GT reconstruction. "
    ...
    p.add_argument("--allow_test_split_training", action="store_true", default=False,
                   help="DEVELOPMENT ONLY. Allow a split whose name starts with 'test' "
                        ...
                        "publication-grade run â€” it breaks the held-out evaluation contract.")
    # bookkeeping
...
def _parse_bento_annot_for_fit(path: str, total_frames: int, fps: float,
    ...
    """Parse a MARS .annot file into a per-frame class-id array.
...
def _build_v25_fit_videos(
    ...
    segment. Uncovered gaps between val windows are NOT collapsed â€”
    ...
    string is honest about the mix:
      "annot_files"                  â€” annot_root provided AND every
                                        used video had its annot file
      "annot_files_partial"          â€” annot_root provided, some videos
                                        used annot, others fell back
                                        to window-label projection
      "annot_files_unavailable"      â€” annot_root provided but ZERO
                                        videos found their annot file
                                        (fit cannot proceed in this mode)
      "window_label_projection"      â€” annot_root not provided
    ...
    if len(val_probs) != len(val_samples):
        print(f"[temporal splitter] WARNING: val_probs has {len(val_probs)} rows but "
              f"val_samples has {len(val_samples)} entries. Cannot align â€” splitter fit aborted.")
    ...
    for source_video, items in by_video.items():
        ...
        # prob_target=0 there (irrelevant â€” segment iteration skips them).
        ...
        if not annot_loaded:
            ...
            # is False â€” i.e., partial mode allows projection per-video).
        ...
        # v25_splitter.py â€” never at storage time.
...
def format_confusion_matrix(cm: np.ndarray, idx_to_class) -> str:
    ...
    lines = []
    lines.append("Confusion matrix (rows=true, cols=pred) â€” counts:")
    ...
    lines.append("")
    lines.append("Confusion matrix â€” row-normalized (recall):")
    lines.append(header)
    for i, s in enumerate(names):
...
def _feature_cache_expected_meta(args, num_frames: int, n_animals: int, num_keypoints: int) -> dict:
    ...
    if int(getattr(args, "precomputed_visual_dim", 0) or 0) <= 0:
        expected.update({
...
def _check_feature_cache(cache_dir: Path, expected: dict):
    manifest_path = cache_dir / "cache_manifest.json"
    if not manifest_path.is_file():
        return False, f"{manifest_path} not found", None
    try:
        meta = json.loads(manifest_path.read_text(encoding="utf-8"))
    except Exception as exc:
        return False, f"could not read {manifest_path}: {exc}", None
    mismatches = []
    for key, expected_value in expected.items():
        ...
        if isinstance(expected_value, int):
            try:
                ...
                ok = False
        elif key in {"manifest_path", "yolo_weights"}:
            try:
                ok = Path(str(actual)).resolve() == Path(str(expected_value)).resolve()
            except Exception:
                ok = str(actual) == str(expected_value)
        else:
            ok = str(actual) == str(expected_value)
        if not ok:
            mismatches.append(f"{key}: cache={actual!r} train={expected_value!r}")
    if mismatches:
        return False, "; ".join(mismatches), meta
    return True, "ok", meta


def _run_auto_feature_cache(args, cache_dir: Path, split_names, num_keypoints: int, overwrite: bool) -> None:
    script_path = Path(__file__).with_name("precompute_visual_features_y.py")
    batch = int(args.feature_cache_batch) if int(args.feature_cache_batch) > 0 else int(args.batch)
    workers = (
        int(args.feature_cache_num_workers)
        if int(args.feature_cache_num_workers) >= 0
    ...
    cmd = [
        ...
        "--splits", *[str(s) for s in split_names],
        "--yolo_backbone_end_layer", str(int(args.yolo_backbone_end_layer)),
        "--n_animals", str(int(args.n_animals)),
        "--num_keypoints", str(int(num_keypoints)),
        "--batch", str(batch),
        "--num_workers", str(workers),
        "--device", str(args.device),
        "--cache_dtype", str(args.feature_cache_dtype),
    ]
    if bool(args.skip_invalid_samples):
        cmd.append("--skip_invalid_samples")
    if bool(args.amp):
        cmd.extend(["--amp", "--amp_dtype", str(args.amp_dtype)])
    if overwrite:
        cmd.append("--overwrite")
    print(
        "[feature cache] building cache before training "
        f"dir={cache_dir} splits={list(split_names)} batch={batch} workers={workers}",
        flush=True,
    )
    subprocess.run(cmd, check=True)


def _missing_feature_cache_count(cache_dir: Path, samples, max_examples: int = 3):
    """Count samples whose cache file is missing.

    Implementation note: a single ``glob('*.npz')`` + set membership is much
    faster than calling ``Path.is_file()`` per sample (which is one syscall
    each, and on Windows + NTFS + AV scanning can take ~1 ms per check). For
    19k samples that's ~20 s shaved off every training launch.
    """
    if not cache_dir.exists():
        # Return all samples as missing using fewer syscalls
        missing = []
        for sample in samples[:max_examples]:
            missing.append(str(feature_cache_path(cache_dir, sample)))
        return len(samples), missing

    cached_names = {p.name for p in cache_dir.glob("*.npz")}
    missing: list[str] = []
    count = 0
...
def main():
    ...
    if schema != "behaviorscope-n-v1":
        raise SystemExit(
            f"Manifest schema_version={schema!r}; expected 'behaviorscope-n-v1'. "
            "Use the legacy trainer for single-animal manifests."
        )
    ...
    # If visual streams are fully disabled (pose-only / relations-only ablations),
    # the model never consumes group_feat / animal_feat â€” the cache is a no-op.
    # Auto-disable both flags so the user can pass --auto_feature_cache uniformly
    # across the ablation matrix without R4 / R4b crashing on the feature_dim
    # mismatch check downstream.
    _visual_streams_off = bool(
        args.disable_visual_streams
        or (args.disable_group_rgb and args.disable_per_animal_rgb)
    )
    if _visual_streams_off:
        print(
            "[visual] streams disabled "
            "(--disable_visual_streams or both RGB streams off); "
            "dataset will skip loading group_frames/animal_frames and the "
            "feature cache will be ignored (model.feature_dim=0).",
            flush=True,
        )
        if args.auto_feature_cache or args.use_feature_cache:
            args.auto_feature_cache = False
            args.use_feature_cache = None
    if args.auto_feature_cache and not args.use_feature_cache:
        args.use_feature_cache = str(Path(args.manifest_path).parent / "yolo_feature_cache")
    if (args.use_feature_cache or args.auto_feature_cache) and args.train_backbone:
        raise SystemExit(
            "Feature caching is only valid with the frozen YOLO backbone. "
            "Remove --train_backbone, or train without cached features."
        )
    ...
    if not args.allow_test_split_training:
        ...
        if _test_requested:
            raise SystemExit(
                ...
                "override (development / debugging only â€” never in publication runs)."
            )
    ...
    if _unused:
        print(f"[manifest] splits not used for training: {_unused} "
              f"(held-out splits remain untouched â€” this is expected)")
    # ---- end split guard ---------------------------------------------------
    print(
    ...
    if args.use_feature_cache:
        cache_dir = Path(args.use_feature_cache)
        expected_cache = _feature_cache_expected_meta(
            args, num_frames=num_frames, n_animals=args.n_animals, num_keypoints=num_keypoints
        )
        ok, reason, cache_meta = _check_feature_cache(cache_dir, expected_cache)
        if not ok and bool(args.allow_feature_cache_without_manifest):
            print(
                "[feature cache] metadata check bypassed by "
                "--allow_feature_cache_without_manifest: "
                f"{reason}. Per-sample cache-file presence will still be verified.",
                flush=True,
            )
            ok = True
            cache_meta = {
                "metadata_bypassed": True,
                "metadata_bypass_reason": reason,
                **expected_cache,
            }
        if not ok and args.auto_feature_cache:
            print(f"[feature cache] cache missing/stale: {reason}", flush=True)
            split_names = list(dict.fromkeys(list(args.train_splits) + list(args.val_splits)))
            _run_auto_feature_cache(
                args,
                cache_dir,
        ...
        if not ok:
            raise SystemExit(
                "Feature cache does not match this training run or is missing: "
                f"{reason}. Run precompute_visual_features_y.py for the same "
                "manifest/model, pass --auto_feature_cache, or omit --use_feature_cache."
            )
        missing_count, missing_examples = _missing_feature_cache_count(
            cache_dir, list(train_samples) + list(val_samples)
        )
        if missing_count and args.auto_feature_cache:
            print(
                f"[feature cache] {missing_count} sample cache files missing; "
        ...
        if missing_count:
            raise SystemExit(
                f"Feature cache is missing {missing_count} sample files. "
                f"Examples: {missing_examples}. Pass --auto_feature_cache to fill them."
            )
        feature_cache_meta = cache_meta
        print(f"[feature cache] ready: {cache_dir}", flush=True)
    ...
    train_ds = MultiAnimalSequenceDataset(
        ...
        feature_cache_dir=args.use_feature_cache,
        require_feature_cache=bool(args.use_feature_cache),
        skip_visual=_visual_streams_off,
    ...
    if args.use_feature_cache:
        print(f"[feature cache] enabled: {args.use_feature_cache}")
    ...
    model = MultiAnimalBehaviorSequenceClassifier(
        ...
        yolo_backbone_end_layer=args.yolo_backbone_end_layer,
        ...
        attention_proj_dim=args.attention_proj_dim,
        positional_encoding=args.positional_encoding,
        pose_fusion_dim=args.pose_fusion_dim,
        pose_fusion_strategy=args.pose_fusion_strategy,
        disable_visual_streams=args.disable_visual_streams,
        disable_group_rgb=args.disable_group_rgb,
        disable_per_animal_rgb=args.disable_per_animal_rgb,
        ...
        precomputed_visual_dim=int(args.precomputed_visual_dim),
    ).to(device)

    n_params = sum(p.numel() for p in model.parameters())
    n_trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    # `feature_dim` reports the visual backbone's output dim. When all
    # visual streams are disabled (R4 / R4b), the backbone is skipped and
    # this prints 0 â€” confirms at-a-glance that no visual encoder was
    # built and no Kinetics/ImageNet weights were loaded.
    print(
    ...
    if feature_cache_meta is not None and "feature_dim" in feature_cache_meta:
        if int(feature_cache_meta["feature_dim"]) != int(model.feature_dim):
            raise SystemExit(
                f"Feature cache feature_dim={feature_cache_meta['feature_dim']} "
                f"but model feature_dim={model.feature_dim}. Rebuild the cache "
                "with the same YOLO weights/backbone layer."
            )
    if int(args.precomputed_visual_dim) > 0 and not args.use_feature_cache:
        raise SystemExit("--precomputed_visual_dim requires --use_feature_cache.")
    ...
    backbone_params = [p for n, p in model.named_parameters() if n.startswith("frame_encoder.") and p.requires_grad]
    head_params = [p for n, p in model.named_parameters() if not n.startswith("frame_encoder.") and p.requires_grad]
    optim_groups = []
    if backbone_params:
        optim_groups.append({"params": backbone_params, "lr": args.backbone_lr})
    optim_groups.append({"params": head_params, "lr": args.lr})
    ...
    config_dump = {
        ...
        "seed": args.seed,
        "manifest_path": str(args.manifest_path),
        "backbone": "precomputed_visual_cache" if int(args.precomputed_visual_dim) > 0 else "yolo",
        "precomputed_visual_dim": int(args.precomputed_visual_dim),
        "yolo_weights": str(args.yolo_weights),
        "yolo_backbone_end_layer": args.yolo_backbone_end_layer,
        "n_animals": args.n_animals,
        ...
        "pose_fusion_strategy": args.pose_fusion_strategy,
        "train_backbone": args.train_backbone,
        "pretrained_backbone": True,  # YOLO weights are always domain-pretrained
        ...
        "feature_cache": {
            ...
            "metadata": feature_cache_meta,
        },
        "model_stats": {
            "total_parameters": int(n_params),
            "trainable_parameters": int(n_trainable),
            "trainable_backbone_parameters": int(sum(p.numel() for p in backbone_params)),
            "trainable_head_parameters": int(sum(p.numel() for p in head_params)),
            "visual_feature_dim": int(model.feature_dim),
            "combined_sequence_dim": int(model.combined_dim),
        },
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        lr_head = lrs[-1] if lrs else float("nan")
        lr_backbone = lrs[0] if len(lrs) > 1 else lr_head
    ...
    # each clip *is* a bout â€” there are no merged-bout regions to split.
    ...
    if args.disable_v25_splitter:
        ...
        splitter_config = make_v25_disabled_config(
            ...
            "dense_bout_annotations is not set â€” every clip is a pre-segmented "
        ...
        try:
            ...
            # correct for non-30-fps datasets.  All MARS recordings are 30 fps
            # so a single unique value is the common case.  Warn on mixed fps.
            _fps_vals = sorted({
            ...
            # Embed audit counts so users can verify fit provenance
    ...
    if stopped_by_flag:
        print("[resume] stop flag was detected; completion marker was not written.")
    else:
        ...
        completion = {
            ...
            "best_model_macro_f1": str(run_dir / "best_model_macro_f1.pt"),
            "config": str(run_dir / "config.json"),
            "training_log": str(history_csv_path),
        }
