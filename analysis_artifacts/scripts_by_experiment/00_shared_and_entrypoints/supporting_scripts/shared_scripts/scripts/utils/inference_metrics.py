"""
Runtime-metrics logger for BehaviorScope-N inference.
...
def _query_nvidia_smi(gpu_idx: int = 0) -> dict | None:
    """Best-effort NVIDIA metrics fallback when pynvml is unavailable."""
    cmd = [
        "nvidia-smi",
        f"--id={int(gpu_idx)}",
    ...
    try:
        return {
            ...
            "gpu_mem_used_mb": round(float(parts[1]), 1),
...
class MetricsLogger:
    ...
    def start(self) -> None:
        ...
        if self._nvml_handle is None:
            if _TORCH_OK and torch.cuda.is_available():
                try:
                    self._gpu_idx = int(torch.cuda.current_device())
                except Exception:
                    self._gpu_idx = 0
            self._nvidia_smi_ok = _query_nvidia_smi(self._gpu_idx) is not None
    ...
    def _sample_gpu_util(self) -> None:
        if self._nvml_handle is not None and _PYNVML_OK:
            try:
                ...
                pass
        # Do not call nvidia-smi here: subprocess startup per window is too
        # expensive. The nvidia-smi fallback samples once per CSV flush.
    ...
    def _sample_system(self) -> dict:
        ...
        if self._nvml_handle is not None and _PYNVML_OK:
            try:
                ...
                pass
        elif self._nvidia_smi_ok:
            sample = _query_nvidia_smi(self._gpu_idx)
            if sample is not None:
                out["gpu_mem_used_mb"] = sample["gpu_mem_used_mb"]
                out["gpu_temp_c"] = sample["gpu_temp_c"]
                if self._gpu_util_samples:
                    out["gpu_util_percent_avg"] = round(
                        sum(self._gpu_util_samples) / len(self._gpu_util_samples), 1)
                    out["gpu_util_percent_max"] = max(self._gpu_util_samples)
                else:
                    out["gpu_util_percent_avg"] = sample["gpu_util"]
                    out["gpu_util_percent_max"] = sample["gpu_util"]
        return out
