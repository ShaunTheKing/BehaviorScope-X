...
``train_y.py --auto_feature_cache`` can consume directly.
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--output_root", type=Path, default=None,
                   help="Default: <dataset_root>/behaviorscope_y_processed.")
...
def stratified_split(
    ...
    for class_samples in by_class.values():
        ...
        rng.shuffle(rows)
        train_y, val_n, test_n = _split_counts(
            len(rows), (train_ratio, val_ratio, test_ratio)
        )
        out["train"].extend(rows[:train_y])
        out["val"].extend(rows[train_y:train_y + val_n])
        out["test"].extend(rows[train_y + val_n:train_y + val_n + test_n])
    return out
...
def main() -> None:
    ...
    output_root = (args.output_root or (args.dataset_root / "behaviorscope_y_processed")).resolve()
    output_root.mkdir(parents=True, exist_ok=True)
    manifest_path = (args.manifest_path or (output_root / "sequence_manifest.json")).resolve()

    logger = setup_logger("PrepareBehaviorScopeY", output_root / "prepare_clips_y.log")
    t0 = time.time()
    ...
    metadata = {
        ...
        "generated_by": "BehaviorScope-Y/prepare_clips_y.py",
    }
    ...
    summary_path = output_root / "prepare_clips_y_summary.json"
...
if __name__ == "__main__":
    try:
        ...
        print(f"[prepare-y] ERROR: {exc}", file=sys.stderr)
        raise
