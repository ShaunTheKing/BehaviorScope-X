...
def collect_pose_backend_provenance(suite_root: Path) -> list[dict]:
    ...
    backend_schemas = suite_cfg.get("visual_backend_schemas", suite_cfg.get("visual_backend_contracts", {}))
    rows: list[dict] = []
    for backend, schema in sorted(backend_schemas.items()):
        row = {
            "visual_backend": backend,
            "role": schema.get("role", ""),
            "pose_model_family": schema.get("pose_model_family", ""),
            "pose_model_backbone": schema.get("pose_model_backbone", ""),
            "pose_model_size": schema.get("pose_model_size", ""),
            "feature_layer": schema.get("feature_layer", ""),
            "pooling": schema.get("pooling", ""),
            "feature_dim_per_crop": schema.get("feature_dim_per_crop", ""),
            "raw_visual_dim_per_frame": schema.get("raw_visual_dim_per_frame", ""),
            "train_cache": schema.get("train_cache", ""),
            "heldout_cache": schema.get("heldout_cache", ""),
        }
        if backend == "mobilenetv3_native":
            cache_manifest = read_json(Path(str(schema.get("train_cache", ""))) / "cache_manifest.json")
            row["parameter_count"] = cache_manifest.get("parameter_count", "")
            row["pose_metrics_source"] = str(Path(str(schema.get("train_cache", ""))) / "cache_manifest.json")
