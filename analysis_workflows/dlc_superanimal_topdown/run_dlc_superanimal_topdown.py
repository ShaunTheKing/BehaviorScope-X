...
APP_ROOT = THIS_DIR.parents[1]
DLC_SCRIPTS = THIS_DIR / "dlc_pipeline_scripts"
...
def resolve_path(value: str | None, base: Path | None = None) -> str:
    ...
    if path.is_absolute():
        return str(path)
    return str(((base or APP_ROOT) / path).resolve())
...
def build_commands(cfg: dict, device: str) -> list[dict]:
    ...
    commands.append({
        ...
        "command": py(
            DLC_SCRIPTS / "dlc_superanimal_train_mars.py",
    ...
    commands.append({
        ...
        "command": py(
            DLC_SCRIPTS / "dlc_topdown_full_video_npz.py",
    ...
    commands.append({
        ...
        "description": "Build pooled multi-resolution HRNet-W32 feature cache for train/validation windows.",
        "command": py(
            DLC_SCRIPTS / "dlc_feature_cache_adapter.py",
    ...
    commands.append({
        ...
        "command": py(
            Path(behaviorscope_scripts) / "train_x.py",
            "--manifest_path", trainval_manifest,
    ...
    commands.append({
        ...
        "command": py(
            DLC_SCRIPTS / "prepare_mars_heldout_mp4_manifest.py",
            "--mars_root", resolve_path(paths["mars_root"]),
    ...
    commands.append({
        ...
        "description": "Build pooled multi-resolution HRNet-W32 feature cache for held-out DLC top-down windows.",
        "command": py(
            DLC_SCRIPTS / "dlc_feature_cache_adapter.py",
    ...
    commands.append({
        ...
        "command": py(
            Path(behaviorscope_scripts) / "eval_manifest_classifier_x.py",
            "--manifest_path", heldout_manifest,
