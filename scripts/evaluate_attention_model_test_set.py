...
V2 held-out test-set evaluation of BehaviorScope-Y R5_yolo_attn_v2_pipeline
(10-epoch v2 run, val_macroF1 0.871) across all videos in test_1 and test_2.

Differences from the v1 script (`evaluate_full_model_test_set.py`):
  - Reads inference outputs from R5_yolo_attn_v2_pipeline_mars_test_eval/
    instead of the v1 R5_yolo_attn_ep10_mars_test_eval/.
  - Writes pooled + per-video CSVs into figures/data/R5_yolo_attn_v2_pipeline_test_eval/.

The v2 R5 checkpoint embeds a val-fitted threshold decoder
(thresholds_by_index = [0.6, 0.4, 0.35, 0.0]) - infer_y.py auto-loads it
from config.json. So predictions consumed here are already
threshold-decoded; this script just measures frame and bout F1 against
.annot ground truth.

Same Bento .annot parser, smoothed-frame loader, frame-level P/R/F1, and
greedy bout matching at tIoU 0.25/0.50 as the v1 script - direct
comparability of v1 vs v2 numbers.
"""
...
INFERENCE_ROOT = (
    r".\BehaviorScope"
    r"\behavior_lstm_runs\R5_yolo_attn_v2_pipeline_mars_test_eval"
)
GT_ROOT = r".\MARS_data"
...
OUT_DIR = (
    r"."
    r"\manuscript_outputs\figures\data\R5_yolo_attn_v2_pipeline_test_eval"
)
