...
try:  # pragma: no cover - package/script dual use
    from cropping import _check_ultralytics, compute_crop_xyxy, normalize_device
...
class NCropDetection:
    ...
    animal_rgbs: np.ndarray  # [N, H, W, 3]
...
def crop_resize_pad_rgb(
    ...
    if sx2 > sx1 and sy2 > sy1:
        dx1 = sx1 - x1
        dy1 = sy1 - y1
        canvas[dy1 : dy1 + (sy2 - sy1), dx1 : dx1 + (sx2 - sx1)] = frame_bgr[sy1:sy2, sx1:sx2]

    resized = cv2.resize(canvas, (int(crop_size), int(crop_size)), interpolation=cv2.INTER_LINEAR)
...
def iterate_yolo_n_crops(
    ...
    *,
    ...
    for frame_idx, result in enumerate(results):
        ...
        cur_k = int(kxy.shape[1]) if kxy.ndim == 3 else 0
        if cur_k > 0:
            if expected_k is None:
                expected_k = cur_k
        if expected_k is not None and cur_k != expected_k:
            kxy = np.zeros((n_animals, expected_k, 2), dtype=np.float32)
            kconf = np.zeros((n_animals, expected_k), dtype=np.float32)
