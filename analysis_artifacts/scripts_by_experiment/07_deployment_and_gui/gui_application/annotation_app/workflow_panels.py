...
def _script_path(name: str) -> Path:
    candidates = [
        THIS_DIR / name,
        REPO_ROOT / "shared_scripts" / "scripts" / name,
        REPO_ROOT / "supporting_scripts" / "shared_scripts" / "scripts" / name,
        THIS_DIR.parent.parent.parent / "shared_scripts" / "scripts" / name,
    ]
...
def _working_directory() -> Path:
    for candidate in [
        ...
        if (candidate / "config.yaml").is_file() or (candidate / "shared_scripts" / "scripts").is_dir():
            return candidate
    return REPO_ROOT
...
class ProcessRunner(QWidget):
    ...
    def _start(self) -> None:
        ...
        process.setWorkingDirectory(str(_working_directory()))
...
class TrainPanel(WorkflowPanel):
    def __init__(self, parent: QWidget | None = None, on_success: Callable[[list[str]], None] | None = None) -> None:
        ...
        self.export_single_model = self._checkbox_row("Export single bundled .pt after training", False)
