...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        default=None,
        help=(
            "Optional YOLO-pose checkpoint fallback. Required only for YOLO-backed "
            "checkpoints that do not embed their YOLO weights; cached MobileNetV3 "
            "and DeepLabCut-HRNet classifiers can load from their feature-cache metadata."
        ),
    )
...
def main() -> int:
    ...
    model, cfg, idx_to_class, num_classes, n_animals, num_keypoints, checkpoint, _visual_source = load_model_from_checkpoint(
        args.model_path,
        args.model_config,
        device=str(device),
        yolo_weights_fallback=str(args.yolo_weights) if args.yolo_weights else None,
    )
