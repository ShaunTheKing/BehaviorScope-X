...
RUNS_ROOT = APP_ROOT / "outputs" / "controlled_comparison_runs"
...
def resolve_path(path: str | Path, *, root: Path = APP_ROOT) -> Path:
    p = Path(path)
...
def validate_math(classical_cfg: dict, tcn_layers: int, tcn_kernel_size: int) -> dict:
    ...
    # Each TCN residual block in model_x.py contains
...
def build_plan(args: argparse.Namespace) -> tuple[list[PlannedCommand], dict]:
    ...
    math_validation = validate_math(classical_cfg, tcn_layers, tcn_kernel)
    ...
    for cache_split, manifest, out_cache, splits_to_cache in [
        ...
        build_cache_cmd = [
            py, str(APP_ROOT / "precompute_mobilenetv3_features_x.py"),
    ...
    for run in neural_runs:
        ...
        eval_neural = [
            py, str(APP_ROOT / "eval_manifest_classifier_x.py"),
    ...
    if bool(args.include_fly):
        ...
        for window_key in fly_windows:
            ...
            commands.append(PlannedCommand(
                ...
                command=[
                    py, str(ROOT / "fly_v_fly" / "fly_to_behaviorscope_npz_full.py"),
        ...
        for eval_key, fly_recipe in sorted(fly_eval_cache_keys.items()):
            ...
            commands.append(PlannedCommand(
                ...
                command=[
                    py, str(APP_ROOT / "precompute_visual_features_x.py"),
        ...
        fly_eval = [
            py, str(ROOT / "fly_v_fly" / "fly_run_cached_eval.py"),
...
def training_command_completed(planned: PlannedCommand, log_root: Path | None) -> bool:
    ...
    train_x.py.  Existing completed commands from a live pre-marker wrapper are
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--mobilenet_checkpoint",
                   default=str(APP_ROOT / "OtherPoseModels" / "mars_mobilenetv3_pose_v6_plus_fullval" / "best_pose_map5095.pt"),
