...
def parse_args():
    ...
    p.add_argument("--single_model_path", default=None,
                   help="Output path for --export_single_model. Default: "
                        "<run_dir>/behaviorscope_y_single_model.pt.")
    ...
    p.add_argument("--amp", action="store_true", default=False,
                   help="Enable automatic mixed precision during training (default off). "
                        "Recorded in config.json; inference AMP is a separate flag in infer_y.py.")
...
def main():
    ...
    # Training and inference AMP are independent choices (see infer_y.py --amp).
    ...
    if args.export_single_model:
        selected_ckpt = run_dir / "best_model_macro_f1.pt"
        if not selected_ckpt.is_file():
            selected_ckpt = run_dir / "best_model.pt"
        single_path = Path(args.single_model_path) if args.single_model_path else run_dir / "behaviorscope_y_single_model.pt"
        try:
