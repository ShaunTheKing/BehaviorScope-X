...
class WorkflowGuideDialog(QDialog):
    def __init__(self, *, show_on_startup: bool, parent: QWidget | None = None):
        ...
        steps = [
            ...
            ("6. Build or reuse feature cache", "Use YOLO/SPPF Cache to precompute frozen visual descriptors."),
            ("7. Train classifier", "Use Train. Training can export a bundled single .pt automatically."),
            ("8. Bundle an existing model", "Model Tools > Bundle existing classifier + pose checkpoint... for models trained earlier."),
            ("9. Run inference", "Use Inference or Batch. Bundled .pt models do not need separate pose weights."),
        ]
...
class AnnotationMainWindow(QMainWindow):
    ...
    def _build_actions(self) -> None:
        ...
        bundle_model_action = QAction("Bundle existing classifier + pose checkpoint...", self)
    ...
    def _build_ui(self) -> None:
        ...
        self.workflow_tabs.setTabToolTip(0, "Annotate + Clip")
        ...
        self.notes_edit.setPlaceholderText("Notes about uncertainty, review comments, or bout context.")
        ...
        self._add_workflow_tab(self.prepare_dataset_panel, "Dataset", "#6A7A89", "Prepare Dataset")
        self._add_workflow_tab(self.prepare_full_video_panel, "Full Video", "#2EA96B", "Prepare Full Video")
        self._add_workflow_tab(self.feature_cache_panel, "YOLO/SPPF", "#6A7A89", "YOLO/SPPF Cache")
    ...
    def _add_workflow_tab(self, widget: QWidget, title: str, color: str, tooltip: str | None = None) -> None:
        index = self.workflow_tabs.addTab(widget, color_swatch_icon(color), title)
        if tooltip:
            self.workflow_tabs.setTabToolTip(index, tooltip)
    ...
    def _is_valid_mars_tutorial_root(self, tutorial_root: Path) -> bool:
        return not self._missing_tutorial_paths(tutorial_root)

    def _missing_tutorial_paths(self, tutorial_root: Path, *, limit: int = 12) -> list[Path]:
        required = [
        ...
        manifest = tutorial_root / "source_manifest.csv"
        if manifest.is_file():
            try:
                with manifest.open("r", encoding="utf-8", newline="") as handle:
                    reader = csv.DictReader(handle)
                    for row in reader:
                        for key in ("video_path", "annot_path"):
                            value = str(row.get(key, "")).strip()
                            if not value:
                                continue
                            path = (tutorial_root / value).resolve()
                            if not path.is_file():
                                missing.append(path)
    ...
    def _resolve_missing_tutorial_data(self, tutorial_root: Path, reason: str) -> Path | None:
        ...
        repo_id = self._tutorial_hf_repo_id()
        download_text = (
            "Download the configured tutorial dataset, or locate a folder that already contains the MP4 files."
            if repo_id
            else "Locate a folder that already contains the tutorial MP4 files."
        )
        box.setInformativeText(
            f"{reason}\n\n{download_text}"
        )
        download_btn = box.addButton("Download", QMessageBox.AcceptRole) if repo_id else None
        locate_btn = box.addButton("Locate Folder", QMessageBox.ActionRole)
        box.addButton(QMessageBox.Cancel)
        box.setDefaultButton(download_btn or locate_btn)
        box.exec()
    ...
    def _load_mars_gui_tutorial(self) -> None:
        ...
        missing = self._missing_tutorial_paths(tutorial_root)
        if missing:
            located = self._resolve_missing_tutorial_data(
                tutorial_root,
                "Could not find the complete tutorial package. Missing examples:\n"
        ...
        QMessageBox.information(
            ...
            (
                ...
                "Full Video, YOLO/SPPF Cache, and Train tabs are filled with tutorial paths. "
                "Prepare the NPZ windows, build the YOLO/SPPF feature cache, then train the "
    ...
    def _bundle_existing_model(self) -> None:
        ...
        if model_config is not None and not model_config.is_file():
            ...
            return
        script = self._script_path("package_single_model_y.py")
    ...
    def _script_path(self, name: str) -> Path:
        ...
        candidates = [
            app_root / name,
            release_root / "supporting_scripts" / "shared_scripts" / "scripts" / name,
            app_root.parent.parent.parent / "shared_scripts" / "scripts" / name,
        ]
    ...
    def _fill_feature_cache_inputs(self, manifest_path: str | None, yolo_weights: str | None, output_root: str | None = None) -> None:
        ...
        if cache_root is None and manifest_path:
            ...
            cache_root = manifest_parent / "yolo_feature_cache"
