...
class MaskedMeanAttentionPool(nn.Module):
    """Small learned-score attention pool over a masked set."""
...
class MultiAnimalBehaviorSequenceClassifier(nn.Module):
    """
    BehaviorScope-N v1 classifier for fixed-taxonomy N-animal behavior.

    Expected input is a dict from SequenceDataset with:
      group_frames [B,T,C,H,W], animal_frames [B,T,N,C,H,W],
      optional pose_features [B,T,N,Dp], masks, reliability, relations.
    """
    ...
    def __init__(
        ...
        relation_input_dim: int = 13,
        relation_dim: int = 128,
        disable_visual_streams: bool = False,
    ):
        super().__init__()
        if str(backbone).lower().startswith("slowfast"):
            raise ValueError("BehaviorScope-N v1 supports frame backbones; use MobileNet/EfficientNet/ViT.")
        ...
        self.group_view_embedding = nn.Parameter(torch.zeros(1, 1, visual_dim))
        self.animal_view_embedding = nn.Parameter(torch.zeros(1, 1, 1, visual_dim))
        nn.init.normal_(self.group_view_embedding, std=0.02)
        nn.init.normal_(self.animal_view_embedding, std=0.02)

        self.pose_fusion = None
        animal_token_dim = visual_dim
        if self.pose_input_dim > 0:
            self.pose_fusion = PoseVisualFusion(
                visual_dim=visual_dim,
                pose_dim=self.pose_input_dim,
                fusion_dim=pose_fusion_dim,
                strategy=pose_fusion_strategy,
                dropout=dropout,
            )
            animal_token_dim = self.pose_fusion.output_dim
        animal_token_dim += 4  # reliability features

        self.animal_pool = MaskedMeanAttentionPool(animal_token_dim, dropout=dropout)
        self.relation_encoder = RelationEncoder(int(relation_input_dim), relation_dim, dropout=dropout)
        self.relation_pool = MaskedMeanAttentionPool(relation_dim, dropout=dropout)

        fused_dim = visual_dim + animal_token_dim + relation_dim
        ...
        self.frame_positional_encoding = (
            ...
            and (temporal_attention_layers > 0 or str(sequence_model).lower() == "attention")
            else None
        )
            ...
            nn.ModuleList(
                [
                    TemporalSelfAttentionBlock(
                        fused_dim, num_heads=attention_heads, dropout=dropout
                    )
                    for _ in range(int(temporal_attention_layers))
                ]
            )
            if int(temporal_attention_layers) > 0
            else None
        )

        self.sequence_model_type = str(sequence_model).lower()
        ...
        if self.sequence_model_type == "lstm":
            ...
            seq_output_dim = fused_dim
        else:
            raise ValueError("sequence_model must be 'lstm' or 'attention'")

        self.attention_pool = (
            TemporalAttentionPooling(seq_output_dim, num_heads=attention_heads, dropout=dropout)
            if self.use_attention_pooling
            else None
        )
        self.head = nn.Sequential(
            nn.LayerNorm(seq_output_dim),
            nn.Dropout(dropout),
            nn.Linear(seq_output_dim, num_classes),
        )
    ...
    def _encode_animal_frames(self, frames: torch.Tensor) -> torch.Tensor:
        # frames [B,T,N,C,H,W]
        b, t, n = frames.shape[:3]
        flat = frames.reshape(b * t * n, *frames.shape[3:])
        feat = self.frame_encoder(flat)
        return feat.reshape(b, t, n, -1)

    def forward(self, inputs: dict[str, torch.Tensor | None]) -> torch.Tensor:
        group_frames = inputs["group_frames"]
        animal_frames = inputs["animal_frames"]
        if not torch.is_tensor(group_frames) or not torch.is_tensor(animal_frames):
            raise ValueError("MultiAnimalBehaviorSequenceClassifier expects tensor inputs.")
        group_feat = self._encode_frames(group_frames) + self.group_view_embedding
        animal_feat = self._encode_animal_frames(animal_frames) + self.animal_view_embedding
        if self.disable_visual_streams:
            group_feat = torch.zeros_like(group_feat)
            animal_feat = torch.zeros_like(animal_feat)

        pose_features = inputs.get("pose_features")
        pose_mask = inputs.get("pose_mask")
        if self.pose_fusion is not None:
            if pose_features is not None:
                b, t, n, _ = animal_feat.shape
                flat_v = animal_feat.reshape(b * t * n, -1)
                flat_p = pose_features.reshape(b * t * n, -1)
                flat_m = pose_mask.reshape(b * t * n) if torch.is_tensor(pose_mask) else None
                fused = self.pose_fusion(flat_v, flat_p, pose_present=flat_m)
                animal_token = fused.reshape(b, t, n, -1)
            else:
                b, t, n, _ = animal_feat.shape
                flat_v = animal_feat.reshape(b * t * n, -1)
                fused = self.pose_fusion(flat_v, None, pose_present=False)
                animal_token = fused.reshape(b, t, n, -1)
        else:
            animal_token = animal_feat

        reliability = inputs.get("animal_reliability")
        if torch.is_tensor(reliability):
            animal_token = torch.cat([animal_token, reliability.to(dtype=animal_token.dtype)], dim=-1)
        else:
            animal_token = torch.cat([animal_token, torch.zeros(*animal_token.shape[:-1], 4, device=animal_token.device, dtype=animal_token.dtype)], dim=-1)

        animal_mask = inputs.get("animal_mask")
        b, t, n, d = animal_token.shape
        pooled_animals = self.animal_pool(
            animal_token.reshape(b * t, n, d),
            animal_mask.reshape(b * t, n) if torch.is_tensor(animal_mask) else None,
        ).reshape(b, t, d)

        relation_features = inputs.get("relation_features")
        relation_reliability = inputs.get("relation_reliability")
        relation_present = inputs.get("relation_present")
        if torch.is_tensor(relation_features):
            rel = relation_features
            if torch.is_tensor(relation_reliability):
                rel = torch.cat([rel, relation_reliability.to(dtype=rel.dtype)], dim=-1)
            else:
                rel = torch.cat([rel, torch.zeros(*rel.shape[:-1], 2, device=rel.device, dtype=rel.dtype)], dim=-1)
            br, tr, nr, _, rd = rel.shape
            rel_tok = self.relation_encoder(rel.reshape(br * tr * nr * nr, rd)).reshape(br, tr, nr * nr, -1)
            rel_mask = relation_present.reshape(br, tr, nr * nr) if torch.is_tensor(relation_present) else None
            pooled_rel = self.relation_pool(rel_tok.reshape(br * tr, nr * nr, -1), rel_mask.reshape(br * tr, nr * nr) if rel_mask is not None else None)
            pooled_rel = pooled_rel.reshape(br, tr, -1)
        else:
            pooled_rel = torch.zeros(group_feat.shape[0], group_feat.shape[1], self.relation_encoder.output_dim, device=group_feat.device, dtype=group_feat.dtype)

        z = torch.cat([group_feat, pooled_animals, pooled_rel], dim=-1)
        if self.frame_positional_encoding is not None:
            z = self.frame_positional_encoding(z)
        if self.temporal_blocks is not None:
            for block in self.temporal_blocks:
                z = block(z)

        if self.sequence_model_type == "lstm":
            seq_out, _ = self.sequence_model(z)
            pooled = self.attention_pool(seq_out) if self.attention_pool is not None else seq_out[:, -1, :]
        else:
