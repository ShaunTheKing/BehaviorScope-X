#!/usr/bin/env python3
"""
mars_to_behaviorscope_npz.py
=============================
Build BehaviorScope-trainable NPZ files + sequence_manifest.json directly
from MARS data, **bypassing YOLO inference entirely**. Uses the MARS
ground-truth pose JSON for keypoints and bboxes, and decodes frames from the
already-chopped per-class .mp4 clips produced by mars_to_behaviorscope_clips.py.
...
Usage (Windows, IntegraPose conda env):
    python mars_to_behaviorscope_npz.py ^
        --mars-root  .\\MARS_data ^
        --clips-root .\\mars_behaviorscope_clips ^
        --out-root   .\\mars_behaviorscope_processed ^
        --emit-mode  single ^
        --window-size 30 --window-stride 15 ^
...
The script:
  ...
  - writes sequence_manifest.json with stratified-by-source-video splits.
"""
...
def main():
    p = argparse.ArgumentParser(description="Build BehaviorScope NPZs directly from MARS pose JSON (no YOLO).")
    ...
    p.add_argument("--source-splits", nargs="+", default=["train", "validation"],
                   help="Which MARS splits the clips were drawn from (used to look up pose JSON).")
    ...
    p.add_argument("--emit-mode", choices=["single", "n_animal"], default="single")
    ...
    p.add_argument("--split-strategy", choices=["video", "window"], default="video")
    ...
    body_length_cache: dict[str, float] = {}
    ...
    for vid_id in needed_video_ids:
        ...
        bl = estimate_body_length_pixels(
            pose_cache[vid_id]["keypoints"],
            pose_cache[vid_id]["bscores"],
            mouse_id=0,
        )
        body_length_cache[vid_id] = float(bl) if bl is not None else 100.0
    ...
    for cidx, clip in enumerate(clip_records, 1):
        ...
        body_len = body_length_cache.get(vid_id, 100.0)
        ...
        H_orig = 570    # MARS top-view default; we'll use frame shape from .mp4 for safety
        W_orig = 1024
        ...
        for w_idx in range(n_windows):
            ...
            if args.emit_mode == "single":
                ...
                # Reuse the keypoint-agnostic extractor from BehaviorScope-Y if importable
                try:
                    sys.path.insert(0, os.path.join(os.path.dirname(_HERE), "BehaviorScope"))
    ...
    if args.split_strategy == "video":
        # Group samples by source_video and class, then assign whole videos to splits
        by_class_video = defaultdict(lambda: defaultdict(list))
        for s in samples:
            by_class_video[s["label"]][s["source_video"]].append(s)
        splits = {"train": [], "val": [], "test": []}
        for label, videos in by_class_video.items():
            ...
            for vid_id in video_ids[:n_train]:
                splits["train"].extend(videos[vid_id])
            for vid_id in video_ids[n_train:n_train + n_val]:
                splits["val"].extend(videos[vid_id])
            for vid_id in video_ids[n_train + n_val:]:
                splits["test"].extend(videos[vid_id])
    else:
        # window-level split (can leak overlapping windows from same video)
        by_class = defaultdict(list)
        for s in samples:
    ...
    calibration = {
        ...
        "per_video_calibration": [
            {"video_id": vid, "split": video_id_to_split.get(vid, "unknown"),
             "body_length_px": float(body_length_cache.get(vid, 100.0))}
    ...
    print(f"  python BehaviorScope/train_behavior_lstm.py \\")
    print(f"      --manifest_path {manifest_path} \\")
    print(f"      --backbone mobilenet_v3_small --use_pose")
