...
def package_single_model_checkpoint(
    *,
    ...
    """Create a single portable .pt containing classifier state and YOLO weights."""
    ...
    cfg = dict(config_override or checkpoint.get("config") or checkpoint.get("model_config") or {})
...
def parse_args():
    ...
    p.add_argument("--yolo_weights", default="",
                   help="Path to a YOLOv8/v11-pose .pt checkpoint. Required for "
                        "YOLO feature extraction, auto feature-cache creation, "
                        "YOLO backbone fine-tuning, and bundled YOLO inference. "
                        "Optional when training strictly from a non-YOLO "
                        "precomputed feature cache such as MobileNetV3 or DLC-HRNet.")
    ...
    p.add_argument("--sequence_model", choices=["lstm", "attention"], default="lstm")
    ...
    p.add_argument("--use_feature_cache", "--user_feature_cache", dest="use_feature_cache", default=None,
                   help="Directory of precomputed YOLO visual features from "
                        "precompute_visual_features_x.py. When set, training loads "
    ...
    p.add_argument("--disable_threshold_decoder", action="store_true",
                   help="Do not fit the validation-calibrated per-class threshold decoder "
                        "after training. By default, train_x.py fits thresholds on the "
    ...
    p.add_argument("--export_single_model", action="store_true",
                   help="After training, write a bundled single .pt containing the "
                        "selected classifier checkpoint, embedded config/decoder metadata, "
                        "and the YOLO-pose weights used for detection and visual features.")
    p.add_argument("--single_model_path", default=None,
                   help="Output path for --export_single_model. Default: "
                        "<run_dir>/behaviorscope_x_single_model.pt.")
    ...
    p.add_argument("--amp", action="store_true", default=False,
                   help="Enable automatic mixed precision during training (default off). "
                        "Recorded in config.json; inference AMP is a separate flag in infer_x.py.")
    ...
    p.add_argument("--allow_test_split_training", action="store_true", default=False,
                   help="DEVELOPMENT ONLY. Allow a split whose name starts with 'test' "
                        ...
                        "publication-grade run; it breaks the held-out evaluation design.")
    # bookkeeping
...
def _feature_cache_expected_meta(args, num_frames: int, n_animals: int, num_keypoints: int) -> dict:
    ...
    if str(args.yolo_weights).strip():
        expected["yolo_weights"] = str(args.yolo_weights)
        expected["yolo_backbone_end_layer"] = int(args.yolo_backbone_end_layer)
    return expected
...
def _run_auto_feature_cache(args, cache_dir: Path, split_names, num_keypoints: int, overwrite: bool) -> None:
    script_path = Path(__file__).with_name("precompute_visual_features_x.py")
...
def main():
    ...
    yolo_num_keypoints = inspect_yolo_keypoint_count(args.yolo_weights) if str(args.yolo_weights).strip() else None
    ...
    # the model never consumes group_feat / animal_feat — the cache is a no-op.
    ...
    if args.auto_feature_cache and not args.use_feature_cache:
        if not str(args.yolo_weights).strip():
            raise SystemExit("--auto_feature_cache requires --yolo_weights. For MobileNetV3 or DLC-HRNet, build the feature cache first and pass --use_feature_cache.")
    ...
    if args.train_backbone and not str(args.yolo_weights).strip():
        raise SystemExit("--train_backbone requires --yolo_weights.")
    ...
    # validation loop. For publication-grade runs, test sets are touched
    ...
    if args.use_feature_cache:
        ...
        if not ok:
            raise SystemExit(
                ...
                f"{reason}. Run precompute_visual_features_x.py for the same "
    ...
    model = MultiAnimalBehaviorSequenceClassifier(
        num_classes=num_classes,
        yolo_weights_path=args.yolo_weights or None,
        ...
        visual_backend=str(feature_cache_meta.get("visual_backend", "yolo")) if feature_cache_meta else "yolo",
    ...
    # this prints 0 — confirms at-a-glance that no visual encoder was
    ...
    if feature_cache_meta is not None and "feature_dim" in feature_cache_meta:
        if int(feature_cache_meta["feature_dim"]) != int(model.feature_dim):
            raise SystemExit(
                ...
                "with the same visual backend and feature-tap profile."
            )
    ...
    # Training and inference AMP are independent choices (see infer_x.py --amp).
    ...
    if args.disable_v25_splitter:
        ...
        try:
            ...
            # Embed audit counts so readers can verify fit provenance.
    ...
    if args.export_single_model and str(args.yolo_weights).strip():
        selected_ckpt = run_dir / "best_model_macro_f1.pt"
        if not selected_ckpt.is_file():
            selected_ckpt = run_dir / "best_model.pt"
        single_path = Path(args.single_model_path) if args.single_model_path else run_dir / "behaviorscope_x_single_model.pt"
        try:
            package_single_model_checkpoint(
                classifier_checkpoint=selected_ckpt,
                yolo_weights=Path(args.yolo_weights),
                output_path=single_path,
                config_override=config_dump,
            )
            print(f"[single model] bundled classifier + YOLO weights -> {single_path}")
        except Exception as exc:
            print(f"[warn] single-model export failed: {exc}")
    elif args.export_single_model:
        print(
            "[single model] skipped bundled YOLO export because this run did not provide --yolo_weights. "
            "Cached MobileNetV3/DLC classifiers should be distributed with their matching feature-cache metadata.",
            flush=True,
        )
