#!/usr/bin/env python3
"""Clean release runner for the final MARS YOLO/SPPF analysis.

This runner intentionally excludes TCN, MobileNetV3, and Fly-v-Fly commands.
It keeps only the final manuscript MARS YOLO/SPPF temporal sequence
classifiers and YOLO/SPPF RF/XGBoost baselines.
"""
...
DEFAULT_SUITE_ROOT = PROJECT_ROOT / "outputs" / "controlled_comparison_runs" / "release_mars_yolo_sppf_ablation"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plan or run the final MARS YOLO/SPPF ablation and YOLO/SPPF classical baselines.",
    ...
    parser.add_argument(
        "--rebuild_npz",
        action="store_true",
        help="Prepend MARS train/held-out NPZ and YOLO/SPPF feature-cache build commands. Default reuses final caches.",
    )
...
def build_controlled_args(args: argparse.Namespace) -> argparse.Namespace:
    return argparse.Namespace(
        ...
        neural_heads=["lstm", "attention"],
        lstm_scope="previous",
        full_model_id="full_lstm256",
        all_heads_on_previous_lstm_models=True,
        mobilenet_checkpoint=str(
            PROJECT_ROOT / "OtherPoseModels" / "mars_mobilenetv3_pose_v6_plus_fullval" / "best_pose_map5095.pt"
        ),
        mobilenet_neural_heads=[],
        mobilenet_model_ids=["full_lstm256"],
        skip_mobilenet_neural=True,
        mobilenet_resize_size=640,
        mobilenet_cache_batch=1,
        mobilenet_cache_num_workers=2,
        mobilenet_cache_dtype="float32",
        include_fly=False,
        fly_heads=[],
        fly_hidden_dims=[],
        fly_window_recipes=[],
...
def is_mars_yolo_sppf_command(command: controlled.PlannedCommand) -> bool:
    if command.stage in {"audit", "summarize"}:
        return True
    if command.stage in {"matrices", "train_classical", "eval_classical"}:
        return "yolo_sppf" in command.name
    if command.stage in {"train_neural", "eval_neural"}:
        return "yolo_sppf" in command.name
    return False
...
def build_npz_commands(args: argparse.Namespace, frozen: dict) -> list[controlled.PlannedCommand]:
    ...
    train_feature_cache = train_npz_root / "yolo_feature_cache"
    heldout_feature_cache = controlled.resolve_profile_path(main_cfg, "eval_feature_cache_root/mars_stride16")
    ...
    build_train = [
        py, str(controlled.ROOT / "shared_scripts" / "scripts" / "prepare_full_video_npz.py"),
    ...
    cache_train = [
        py, str(controlled.ROOT / "shared_scripts" / "scripts" / "precompute_visual_features_y.py"),
        "--manifest_path", str(train_manifest),
        "--yolo_weights", str(yolo_weights),
        "--output_dir", str(train_feature_cache),
        "--splits", "train", "val",
        "--yolo_backbone_end_layer", str(recipe["yolo_backbone_end_layer"]),
        "--n_animals", "2",
        "--batch", str(heldout.get("feature_cache_batch", 16)),
        "--num_workers", str(heldout.get("feature_cache_num_workers", 0)),
        "--device", device,
    ]
    controlled.append_amp(cache_train, main_cfg, device)
    ...
    cache_heldout = [
        ...
        "--yolo_weights", str(yolo_weights),
        "--output_dir", str(heldout_feature_cache),
        "--splits", "train", "val",
        "--yolo_backbone_end_layer", str(recipe["yolo_backbone_end_layer"]),
        "--n_animals", "2",
        "--batch", str(heldout.get("feature_cache_batch", 16)),
    ...
    controlled.append_amp(cache_heldout, main_cfg, device)
    ...
    return [
        controlled.PlannedCommand(
            ...
            reason="Build MARS train/validation full-video windows for the final YOLO/SPPF classifier matrix.",
        ),
        controlled.PlannedCommand(
            stage="build_npz",
            name="cache_mars_training_yolo_sppf_features",
            command=cache_train,
            expected_outputs=[str(train_feature_cache / "cache_manifest.json")],
            reason="Precompute frozen YOLO/SPPF descriptors once for all MARS train/validation classifiers.",
        ),
        controlled.PlannedCommand(
            stage="build_npz",
        ...
        controlled.PlannedCommand(
            stage="build_npz",
            name="cache_mars_heldout_yolo_sppf_features",
            command=cache_heldout,
            expected_outputs=[str(heldout_feature_cache / "cache_manifest.json")],
            reason="Precompute frozen YOLO/SPPF descriptors once for held-out classifier-only evaluation.",
        ),
    ]
...
def main() -> int:
    ...
    selected = [
        ...
        for command in commands
        if is_mars_yolo_sppf_command(command) and stage_selected(command, args.stage)
    ]
    ...
    print(f"Release plan: {THIS_DIR / 'release_command_plan.txt'}")
    print(f"Suite root: {args.suite_root}")
    print(f"Selected MARS YOLO/SPPF stage: {args.stage} ({len(selected)} commands)")
    ...
    return controlled.run_commands(
        ...
        skip_completed=bool(args.skip_completed),
        log_root=args.suite_root / "logs_mars_yolo_sppf",
        keep_going=bool(args.keep_going),
