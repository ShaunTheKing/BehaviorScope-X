...
``BehaviorScope-Y_v5/PLAN.md`` and
``research_notebook_y/19_methodology_lessons_clip_vs_fullvideo.qmd``.
"""
...
# Reuse `.annot` parsing + `.seq` conversion from the existing scripts/ modules.
# The v4 batch driver already has battle-tested versions; importing keeps a
...
except ImportError:  # final self-contained folder uses the local canonical name
...
@dataclass
class VideoSource:
    split: str         # which v5 split this video belongs to: 'train' or 'val'
...
def discover_sources(
    ...
    *,
    ...
    for dataset_split, mars_split_list in (("train", train_splits), ("val", val_splits)):
        for mars_split in mars_split_list:
            ...
            for vid_dir in sorted(split_dir.iterdir()):
                ...
                annot = find_annot(vid_dir, allow_pred_named=allow_pred_named_annots)
    ...
    # and val splits in the v5 sense. (Possible if user passes overlapping
    ...
    if overlap:
        raise SystemExit(
            ...
            f"train and val v5 splits. Use disjoint --train_splits / --val_splits. "
...
def process_video(
    ...
    if src.source_path is not None:
        ...
        if args.source_mp4_cache:
            mp4_path = Path(args.source_mp4_cache) / src.mars_split / f"{safe_stem}.mp4"
        else:
            mp4_path = src.video_dir / f"{src.seq_path.stem}.mp4"
        seek = src.seq_path.with_name(src.seq_path.stem + "-seek.mat")
        ...
        if int(info.get("bad_frames_replaced") or 0):
            logger.warning(
                f"[{src.video_id}] replaced {int(info['bad_frames_replaced'])} unreadable .seq frames "
                f"during MP4 conversion; first_bad_frames={info.get('first_bad_frames', [])}"
            )
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--mars_root", type=Path, default=Path("MARS_data"))
    ...
    p.add_argument("--source_mp4_cache", type=Path, default=None,
                   help="Optional MP4 cache root. Layout: <cache>/<mars_split>/<video_id>.mp4")
    ...
    p.add_argument("--skip_existing", action=argparse.BooleanOptionalAction, default=True,
                   help="Skip only videos with a completed-video marker matching current build settings. "
                        "Use --no-skip_existing to force rebuilding every discovered video.")
