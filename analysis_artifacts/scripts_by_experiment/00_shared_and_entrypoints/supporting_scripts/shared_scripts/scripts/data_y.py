...
def feature_cache_path(cache_dir: Path | str, sample: MultiAnimalSequenceSample) -> Path:
    ...
    Hash key uses only sample.id + frame range (no absolute paths) so the
    cache is portable across machines and folder locations.
    """
    cache_dir = Path(cache_dir)
    key = f"{sample.id}|{sample.start_frame}|{sample.end_frame}"
...
def validate_n_npz_payload(
    ...
    *,
    ...
    """Fail-fast validator for BehaviorScope-N NPZ windows.

    This checks the tensor contract that training and inference assume. It is
...
class MultiAnimalSequenceDataset(Dataset):
    """Loads BehaviorScope-N NPZs and returns multi-stream tensors with masks.
                ...
                if not cache_path.is_file():
                    if self.require_feature_cache:
                        raise FileNotFoundError(
                            ...
                            "Run precompute_visual_features_y.py first, or omit --use_feature_cache."
                        )
                else:
