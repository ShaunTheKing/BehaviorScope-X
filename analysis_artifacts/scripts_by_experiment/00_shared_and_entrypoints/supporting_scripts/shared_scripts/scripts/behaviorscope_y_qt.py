...
def _warn_missing_deps(window: AnnotationMainWindow) -> None:
    ...
    if shutil.which("ffmpeg") is None:
        missing.append(
            "- ffmpeg - optional for faster clip extraction in the 'Annotate + Clip' tab.\n"
            "  If it is unavailable, BehaviorScope-Y falls back to OpenCV extraction.\n"
            "  Install: https://ffmpeg.org/download.html\n"
            "  Windows quick install:  winget install ffmpeg\n"
            "              or:  choco install ffmpeg\n"
            "  After installing, restart BehaviorScope-Y to use ffmpeg."
        )
    if missing:
        body = (
            "The following system tools were not found on your PATH:\n\n"
            + "\n\n".join(missing)
            + "\n\nThe app remains usable without these optional tools."
        )
        QMessageBox.information(window, "Optional system tools not found", body)
