...
def _extract_clip_opencv(
    *,
    ...
    if not cap.isOpened():
        raise RuntimeError(f"OpenCV could not open source video: {video.path}")

    source_fps = float(video.fps or cap.get(cv2.CAP_PROP_FPS) or normalized_fps or 30.0)
    start_frame = int(annotation.start_frame)
    end_frame = int(annotation.end_frame)
    if end_frame < start_frame:
        start_frame = max(0, int(round((float(annotation.start_ms) / 1000.0) * source_fps)))
        end_frame = max(start_frame, int(round((float(annotation.end_ms) / 1000.0) * source_fps)) - 1)
    ...
    if not ok or frame is None:
        cap.release()
        raise RuntimeError(
            f"OpenCV could not read frame {start_frame} from source video: {video.path}"
        )
    ...
    writer = cv2.VideoWriter(
        ...
        float(normalized_fps or source_fps or 30.0),
        (int(width), int(height)),
    )
    if not writer.isOpened():
        cap.release()
        raise RuntimeError(f"OpenCV could not create output clip: {clip_path}")

    frame_idx = max(0, start_frame)
    try:
        while ok and frame is not None and frame_idx <= end_frame:
            writer.write(frame)
            frame_idx += 1
