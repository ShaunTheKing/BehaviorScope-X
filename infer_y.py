...
Usage examples:
    ...
    python BehaviorScope\\infer_y.py ^
        --model_path  runs\\behaviorscope_y\\best_model_macro_f1.pt ^
        --model_config runs\\behaviorscope_y\\config.json ^
...
def _extract_embedded_yolo_weights(checkpoint: object, checkpoint_path: Path | str) -> str | None:
    ...
    cache_dir = Path(tempfile.gettempdir()) / "behaviorscope_y_embedded_yolo"
...
def load_model_from_checkpoint(
    ...
    if config_json is not None:
        ...
        raise FileNotFoundError(
            "A train_y.py config.json or a checkpoint with embedded config is "
    ...
    # distributable .pt to carry both the pose model and the classifier.
    ...
    if not yolo_weights_path:
        raise ValueError(
            ...
            "package_single_model_y.py or train_y.py --export_single_model."
        )
    ...
    return model, cfg, idx_to_class, num_classes, n_animals, num_keypoints, checkpoint, str(yolo_weights_path)
...
def main():
    ...
    model_yolo = load_yolo_model(yolo_weights_path, device=device, task=args.yolo_task)
